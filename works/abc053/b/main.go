...
func main() {
	...
	s := inputString()
	var aIndexList, zIndexList []int
	for index, value := range s {
		if string(value) == "A" {
			aIndexList = append(aIndexList, index)
		}
		if string(value) == "Z" {
			zIndexList = append(zIndexList, index)
		}
	}
	outputResult(zIndexList[len(zIndexList)-1] - aIndexList[0] + 1)
}
