...
func main() {
	...
	var xListSlice [][]int
	for i := 0; i < n; i++ {
		xList := inputIntSlice(d)
		xListSlice = append(xListSlice, xList)
	}

	var count int
	for i := 0; i < len(xListSlice); i++ {
		for j := i + 1; j < len(xListSlice); j++ {
			var sum int
			for k := 0; k < d; k++ {
				sum = sum + (xListSlice[i][k]-xListSlice[j][k])*(xListSlice[i][k]-xListSlice[j][k])
			}
			squrt := sqrt(sum)
			if isInteger(squrt) {
				count++
