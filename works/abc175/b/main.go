...
func main() {
	...
	n := inputInt()
	lList := inputIntSlice(n)

	var count int
	sortedLList := sortIntSliceAsc(lList)

	for i := 0; i < len(sortedLList); i++ {
		for j := i + 1; j < len(sortedLList); j++ {
			for k := j + 1; k < len(sortedLList); k++ {
				if sortedLList[i] == sortedLList[j] {
					continue
				}
				if sortedLList[j] == sortedLList[k] {
					continue
				}
				if sortedLList[i] == sortedLList[k] {
					continue
				}
				if sortedLList[i]+sortedLList[j] > sortedLList[k] {
					count++
				}
			}
		}
